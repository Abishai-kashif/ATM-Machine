	...
	while (true) {
		...
		if (selectedOperation.operation === "Deposit Cash") {
			...
			if (deposit.amount1 != 0) {
				if (deposit.amount1 <= depositLimit) {
					...
					console.log(
						chalk.white.bold(
							`\n-> Congrats! You have deposited : ${chalk.greenBright(
								`${deposit.amount1}$`
							)}.`
						)
					);
					...
					console.log(
						chalk.white.bold(
							`-> Your current balance is : ${chalk.greenBright(
								`${currentBalance}$`
							)}.`
						)
					);
					...
					console.log(
						chalk.red(
							`-> You exceeded your maximum deposit limit : ${depositLimit}$`
						)
					);
				}
			} else {
				console.log(
					chalk.red("-> For deposit amount must be greater than zero.")
				);
			}
		}
		...
		if (selectedOperation.operation === "Withdraw Cash") {
			...
			if (selectedMethod.method === "Fast Cash") {
				...
				if (selectedAmounts.selectedAmount <= currentBalance) {
					...
					console.log(
						chalk.white.bold(
							`\n-> Congrats! You have withdrawn : ${chalk.greenBright(
					...
					if (currentBalance === 0) {
						console.log(
							chalk.white.bold(
								`-> Your remaining balance is ${chalk.red(
									`${currentBalance}$`
								)}.`
							)
						);
						...
						console.log(
							chalk.white.bold(
								`-> The remaining balance is : ${chalk.greenBright(
									`${currentBalance}$`
								)}.`
							)
						);
				...
				if (deduct.amount != 0) {
					if (deduct.amount <= currentBalance) {
						...
						if (currentBalance === 0) {
							...
							console.log(
								chalk.white.bold(
									`-> Your remaining balance is : ${chalk.greenBright(
										`${currentBalance}$`
									)}.`
								)
							);
					...
					console.log(
						chalk.red("-> For withdrawal amount must be greater than zero.")
					);
				}
			}
		}
		...
		if (selectedOperation.operation === "Current Balance") {
			if (currentBalance === 0) {
				console.log(
					chalk.white.bold(
						`-> Your current balance is ${chalk.red(`${currentBalance}$`)}.`
					)
				);
			} else {
				console.log(
